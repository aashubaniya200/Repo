...
import 'package:webpal_commerce/controllers/eCommerce/voucher/voucher_controller.dart';
...
import 'package:webpal_commerce/models/eCommerce/voucher/voucher_apply_model.dart';
...
final selectedProductQuantity = StateProvider<int>((ref) => 1);
...
class ShopIdsNotifier extends StateNotifier<Set<int>> {
  ...
  void addAllShopIds() {
    ...
    // Clear existing state and add all shop IDs fresh
    final newShopIds = <int>{};
    for (CartItem cartItem in cartItems) {
      newShopIds.add(cartItem.shopId);
    }
...
class SubTotalNotifier extends StateNotifier<Map<String, dynamic>> {
  ...
  Future<void> calculateSummery({String? couponCode}) async {
    double subTotal = 0;
    double discount = 0;
