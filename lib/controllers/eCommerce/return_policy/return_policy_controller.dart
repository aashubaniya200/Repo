...
import 'package:webpal_commerce/models/eCommerce/common/common_response.dart';
import 'package:webpal_commerce/models/eCommerce/order/order_details_model.dart';
import 'package:webpal_commerce/models/eCommerce/return_policy/return_order_details_model/return_order_details_model.dart';
import 'package:webpal_commerce/models/eCommerce/return_policy/return_order_list_model/return_order_list_model.dart';
import 'package:webpal_commerce/models/eCommerce/return_policy/return_order_submit_model.dart';
import 'package:webpal_commerce/services/eCommerce/return_product/return_product_service.dart';
  ...
  Future<void> fetchReturnOrders() async {
    try {
      ...
      if (response.data is String && (response.data as String).contains('<!DOCTYPE html>')) {
        throw Exception('API returned invalid response. Please check your authentication or contact support.');
      }
      ...
      // Handle DioException properly to show user-friendly error message
      String errorMessage;
      if (e is DioException) {
        errorMessage = ApiInterceptors.handleError(e);
      } else if (e.toString().contains('HTML') || e.toString().contains('authentication')) {
        errorMessage = 'Unable to fetch return orders. Please try logging in again.';
      } else {
        errorMessage = 'Failed to load return orders. Please try again.';
      }
